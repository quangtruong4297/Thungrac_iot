...
void loop() {
  // DEBUG_PRINTLN("test");
// timer thoi gian doc HC va load du lieu esp32 cam
...
// timer thoi gian reconnect server mqtt
  if (TimeReached(time_5second)) {
    SetNextTimeInterval(time_5second, step_5s * 1000);
    if (!mqtt.connected()) {
      DEBUG_PRINTLN("=== MQTT NOT CONNECTED ===");
      if (mqttConnect()) {
        DEBUG_PRINTLN("=== MQTT reconnect oki ===");
      }
      {
        DEBUG_PRINTLN("=== MQTT reconnect not oki ===");
      }
    }
  }
// timer thoi gian doc cam bien GPS
  if (TimeReached(time_30second)) {
    SetNextTimeInterval(time_30second, step_30s * 1000);
    smartDelay(100);
    printGPSInfo();
  }
// timer thoi gian gui du lieu len server
